...
// RMS
/**
 * csrfFreeRoutes that do not require csrf token.
 */
const csrfFreeRoutes = {
  // '/api/upload': true,
  // '/forgot': true,
  // '/login/remote': true,
  // '/mobile/login': true
...
// RMS
var mongo_db_uri = process.env.LOCALHOST_URI;
if (process.env.DATABASE !== "localhost") {
  mongo_db_uri = process.env.PRODUCTION_DB;
}
...
// RMS
/**
 * configure i18n multi language support on server.
 */
i18n.configure({
  locales: ["en", "de"],
...
app.use((req, res, next) => {
  if (req.path === "/api/upload") {
    ...
    // RMS
    // check for non variable part of path
    var csrfFree = false;
    if (!(req.path.indexOf("mobile") == -1)) {
      csrfFree = true;
      config.mobile = true;
    } else {
      for (var i = 0; i < csrfFreeRoutes.length; i++) {
        if (csrfFreeRoutes.indexOf(req.path) !== -1) {
          csrfFree = true;
          break;
        }
      }
